...
singularity exec -e $CODENAME.img dch --create --package "gmd-geodesic-mesh" --distribution $CODENAME --newversion=$VERSION "Build for $(git rev-parse HEAD)"
...
deb-s3 upload --bucket atmosfoam-apt --codename=$CODENAME --component=dev ../gmd-geodesic-mesh_${VERSION}_amd64.deb

# ensure that the new .deb can be installed
sudo singularity exec -e -w $CODENAME.img apt-get update -qq
sudo singularity exec -e -w $CODENAME.img apt-get install gmd-geodesic-mesh=$VERSION -y --allow-unauthenticated --no-install-recommends
